...
//html company select
ob_start();
$company = mysqli_query ( $conn_admin_db, "SELECT id, UPPER(name) FROM company WHERE id IN (SELECT company_id FROM u_motorcycle WHERE status='1') AND status='1' ORDER BY name ASC");
        ...
        <div class="col-md-12">
            <div class="card" id="printableArea">
                <div class="card-header text-center">
                    <strong class="card-title">COMPARISON OF MOTORCYCLE USAGE</strong>
                </div>
                ...
                <div class='row col-sm-12'>
                   ...
                   <!-- <div class='col-sm-2'><label for='motor' class='form-control-label'><small class='form-text text-muted'>Motorcycle</small></label></div>    -->          
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {        
        ...
        $('.btn_compare').on("click", function(event){ 
        	...
        	$.ajax({  
                url:"motorcycle.report.ajax.php",  
                type:"POST",                        
        ...
        function addRow(){
         ...
         //to update the motor list when company change
...
//         	});  
        ...
        //to get the motor based on the selected company
        function get_motor(company_id){
            $.ajax({
                url: 'motorcycle.report.ajax.php',
                type: 'POST',
                data: {action:'get_motor', company: company_id},
                async:false,
            }).done(function( indata ){                             	
