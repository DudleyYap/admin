<?php 
    ...
    $query = "SELECT * FROM u_telekom_account WHERE u_telekom_account.id = '$id'";
    ...
    $details_query = "SELECT u_telekom.id AS bt_id, acc_id, MONTH(date_end) As month, MONTHNAME(date_end) AS month_name, date_start, date_end, due_date,paid_date, 
                    credit_adjustment, rebate, gst_sst, adjustment, cheque_no, bill_no, amount, monthly_bill FROM u_telekom                     
                    WHERE acc_id = '$acc_id' AND YEAR(date_end) = '$year_select' AND u_telekom.status='1' ORDER BY date_end";
    ...
    foreach ($t_list as $key => $val){ 
		$tel_no = itemName("SELECT tel_no FROM u_telefon_list WHERE id='$key'");	
		$type = itemName("SELECT phone_type FROM u_telefon_list WHERE id='$key'");	
        $count++;
        $tel_arr[] = $count;
    ...
    function get_tel_list_usage($bt_id, $month){
        ...
        $qry = "SELECT MONTHNAME(date) AS month_name, u_telefon_list.*, u_telefon_usage.* FROM u_telefon_list
            LEFT JOIN u_telefon_usage ON u_telefon_usage.telefon_id = u_telefon_list.id
            WHERE u_telefon_usage.bt_id='".$bt_id."' AND u_telefon_list.status='1' AND MONTH(date)='$month'";
    ...
    function get_active_tel_list($acc_id){ // to populate the header
        global $conn_admin_db;
        $query = "SELECT id, tel_no FROM u_telefon_list WHERE acc_id = '$acc_id' AND status='1'";
        	...
        	<div class="col-md-12">
                <div class="card" id="printableArea">
                    <div class="card-header">                            
                        <strong class="card-title"><a href="u_telekom_setup.php">TELEKOM</a> > Account Details</strong>
                    </div>     
                    <div class="card-body">                                       
                        <div class="col-sm-12">
