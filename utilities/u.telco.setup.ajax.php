...
function update_account($data){
    ...
    if(!empty($data)){
        ...
        $query = "UPDATE u_telco_account SET
                        company_id='$company',
                        account_no='$acc_no',
                        position='$position',
                        remark='$remark',
...
function add_new_bill($data){
    ...
    if (!empty($data)) {
        ...
        $query = "INSERT INTO u_telco_bill SET
...
function add_new_account($data, $telefon_list){
    ...
    if (!empty($data)) {
        ...
        $query = "INSERT INTO u_telco_account SET
                        company_id='$company',
                        account_no='$acc_no',
                        position='$position',
                        remark='$remark',
...
function retrieve_account($id){
    global $conn_admin_db;
    if (!empty($id)) {        
        $query = "SELECT * FROM u_telco_account WHERE id = '$id'";
...
function delete_account($id){
    global $conn_admin_db;
    if (!empty($id)) {
        $query = "UPDATE u_telco_account SET status = 0 WHERE id = '".$id."' ";
        $result = mysqli_query($conn_admin_db, $query);
        if ($result) {
            alert ("Deleted successfully", "u_telco_setup.php");
        }
    }
}
