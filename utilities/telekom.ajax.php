...
function delete_item_data($id){
    global $conn_admin_db;
    if (!empty($id)) {
        $query = "UPDATE u_telekom SET status = 0 WHERE id = '".$id."' ";
...
function retrieve_item_data($id){
    global $conn_admin_db;
    $query = "SELECT * FROM u_telekom WHERE id='$id'";
...
function get_telefon_usage($bt_id){
    global $conn_admin_db;
    $query = "SELECT u_telefon_usage.id, usage_rm, tel_no, phone_type FROM u_telefon_usage 
            INNER JOIN u_telefon_list ON u_telefon_list.id = u_telefon_usage.telefon_id
            WHERE u_telefon_usage.bt_id='".$bt_id."'";  
...
function get_telekom_data_monthly_compare($year, $company, $account_no){
    ...
    global $month_map;
    $month = 12;
    $query = "SELECT * FROM u_telekom_account
        INNER JOIN u_telekom ON u_telekom_account.id = u_telekom.acc_id
    ...
    if(!empty($account_no)){
        $query .=" AND u_telekom_account.id = '$account_no'";
    }
...
function get_account($company){
    global $conn_admin_db;
    $query = "SELECT id,UPPER(account_no) as account_no FROM u_telekom_account WHERE company_id='$company' AND status='1'";
    $result = mysqli_query($conn_admin_db, $query);
    $acc_arr = array();
    $acc_list = [];
...
function update_account($data){
    ...
    $query = "UPDATE u_telekom_account SET
                        company_id='$company',
                        account_no='$acc_no',
                        owner='$owner',
                        remark='$remark',
...
function update_bill($data){
    ...
    if(!empty($phone_data) && $result){
        foreach ($phone_data as $key => $value){
            $qry = "UPDATE u_telefon_usage SET usage_rm='$value' WHERE id='$key'";
...
function add_new_bill($data){
    ...
    $adjustment = number_format(($rounded-$amount), 2);
    $total_amt = $amount + $adjustment;
    $query = "INSERT INTO u_telekom (acc_id, bill_no, cheque_no, date_start, date_end, paid_date, due_date, monthly_bill, credit_adjustment,gst_sst, amount, rebate, adjustment, other_charges)
    ...
    if(!empty($phone_usage) && $result){
        ...
        $qry = "INSERT INTO u_telefon_usage (telefon_id, bt_id, date, date_added, usage_rm) VALUES".$values;
...
function retrieve_telefon_list($acc_id){
    global $conn_admin_db;
    $query = "SELECT * FROM u_telefon_list WHERE acc_id='$acc_id' AND status='1'";
    $result = mysqli_query($conn_admin_db, $query) or die(mysqli_error($conn_admin_db));
    $data = [];
    while ($row = mysqli_fetch_assoc($result)) {
        $data[] = $row;
...
function add_new_telefon($acc_id, $telefon_list){
    ...
    $query = "INSERT INTO u_telefon_list (acc_id, tel_no, phone_type) VALUES" .$values;
    $result = mysqli_query($conn_admin_db, $query) or die(mysqli_error($conn_admin_db));
    echo json_encode($result);
}
...
function add_new_account($data, $telefon_list){
    ...
    if (!empty($data)) {
        ...
        $query_insert_telekom = "INSERT INTO u_telekom_account SET
                        company_id='$company',
                        account_no='$acc_no',
                        owner='$owner',
                        remark='$remark',
        ...
        if(!empty($telefon_list)){
            ...
            $values = implode(",", $values);
            $query = "INSERT INTO u_telefon_list (bt_id, tel_no, phone_type) VALUES" .$values;
...
function retrieve_account($id){
    global $conn_admin_db;
    if (!empty($id)) {        
        $query = "SELECT * FROM u_telekom_account WHERE id = '$id'";
...
function delete_account($id){
    global $conn_admin_db;
    if (!empty($id)) {
        $query = "UPDATE u_telekom_account SET status = 0 WHERE id = '".$id."' ";
        $result = mysqli_query($conn_admin_db, $query);
        if ($result) {
            alert ("Deleted successfully", "u_telekom_setup.php");
        }
    }
}
