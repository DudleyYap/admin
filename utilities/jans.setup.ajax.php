...
function retrieve_account_details($id){
    global $conn_admin_db;
    if (!empty($id)) {
        $query = "SELECT * FROM u_jans_bill WHERE id = '$id'";
...
function delete_account_details_item($id){
    global $conn_admin_db;
    if (!empty($id)) {
        $query = "DELETE FROM u_jans_bill WHERE id = '".$id."' ";
        $result = mysqli_query($conn_admin_db, $query);
        if ($result) {
            alert ("Deleted successfully", "jans_account_details.php");
        }
    }
}
...
function add_new_bill($data){
    ...
    if (!empty($data)) {
        ...
        $query_insert_ja = "INSERT INTO u_jans_bill
                    SET acc_id = '$acc_id',
...
function add_new_account($data){
    ...
    if (!empty($data)) {
        ...
        $query = "INSERT INTO u_jans_account
...
function retrieve_account($id){
    global $conn_admin_db;
    if (!empty($id)) {        
        $query = "SELECT * FROM u_jans_account WHERE id = '$id'";
        $rst  = mysqli_query($conn_admin_db, $query)or die(mysqli_error($conn_admin_db));
...
function delete_account($id){
    global $conn_admin_db;
    if (!empty($id)) {
        $query = "UPDATE u_jans_account SET status = 0 WHERE id = '".$id."' ";
        $result = mysqli_query($conn_admin_db, $query);
        if ($result) {
            alert ("Deleted successfully", "jans_setup.php");
        }
    }
}
